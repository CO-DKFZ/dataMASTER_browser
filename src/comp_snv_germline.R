COMPONENT$snv_germline = list()

experiment_id = "snv_germline"
experiment_name = "SNV germline"

snv_germline_function_type = c("synonymous", "nonsynonymous", "stopgain", "stoploss", 
	"splice donor", "splice acceptor", "stop retained", "startloss", 
	"incomplete terminal codon", "unknown")
names(snv_germline_function_type) = snv_germline_function_type
	...
	div(
		column(7,
			selectInput(qq("@{experiment_id}_config_function_type"), label = "Function type", 
				choices = snv_germline_function_type, selected = c("nonsynonymous", "stopgain", "stoploss", 
					"splice donor", "splice acceptor", "stop retained", "startloss", 
	...
	$("#@{experiment_id}_top_genes_n_top").change(function() {
		...
		observeEvent(input[[qq("@{experiment_id}_top_genes_n_top")]], {
			...
			output[[qq("@{experiment_id}_top_genes_oncoprint")]] = renderPlot({
				ht = oncoPrint(list(snv_germline = mm), name = experiment_name,
					alter_fun = list(snv_germline = alter_graphic("rect", width = 1, height = 0.9, fill = "red")),
	...
	$("#@{experiment_id}_top_samples_n_top").change(function() {
		...
		observeEvent(input[[qq("@{experiment_id}_top_samples_n_top")]], {
			...
			output[[qq("@{experiment_id}_top_samples")]] = renderTable({
				dt = data.frame(Sample = names(x)[ind], "#SNV_germlines" = xn[ind], "#Mutated genes" = x[ind], check.names = FALSE)
				# dt[, 1] = qq("<a href=\"#\" onclick=\"link_to_other_tab($(this), 'gene', {gene_search:'@{dt[,1]}'}, 'gene_search_submit');false\">@{dt[,1]}</a>", collapse = FALSE)
				dt
			}, sanitize.text.function = function(x) x)
