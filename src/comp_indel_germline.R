...
experiment_id = "indel_germline"
experiment_name = "Indel_germline"

COMPONENT[[experiment_id]] = list()

df = as.data.frame(DB[[experiment_id]])

indel_germline_function_type = unique(as.vector(df$Function))
indel_germline_function_type = indel_germline_function_type[!is.na(indel_germline_function_type)]
names(indel_germline_function_type) = indel_germline_function_type
	...
	div(
		column(7,
			selectInput(qq("@{experiment_id}_config_function_type"), label = "Function type", 
				choices = indel_germline_function_type, selected = setdiff(indel_germline_function_type, c("unknown", "synonymous")), 
	...
	$("#@{experiment_id}_top_samples_n_top").change(function() {
		...
		observeEvent(input[[qq("@{experiment_id}_top_samples_n_top")]], {
			...
			output[[qq("@{experiment_id}_top_samples")]] = renderTable({
				dt = data.frame(Sample = names(x)[ind], "#Indel_germlines" = xn[ind], "#Mutated genes" = x[ind], check.names = FALSE)
