...
COMPONENT[[experiment_id]]$server = local({
	...
	observeEvent(input[[qq("@{experiment_id}_config_function_type")]], {
		...
		if(length(input[[qq("@{experiment_id}_config_function_type")]]) == 0) {
			updateSelectInput(session, qq("@{experiment_id}_config_function_type"), selected = c("CNN", "DUP", "DEL", "AMP", "HDEL"))
			function_type = c("CNN", "DUP", "DEL", "AMP", "HDEL")
		} else {
	...
	observeEvent(input[[qq("@{experiment_id}_config_submit")]], {
		...
		output[[qq("@{experiment_id}_summary_ui")]] = renderUI({
			div(
				...
				box(title = "LOH",
					plotOutput(qq("@{experiment_id}_summary_LOH"))
				),
				box(title = "Function",
					plotOutput(qq("@{experiment_id}_summary_function"))
				),
				box(title = "Distribution of TCN",
					plotOutput(qq("@{experiment_id}_summary_TCN"))
				),
				box(title = "Distribution of coverage ratio",
					plotOutput(qq("@{experiment_id}_summary_CovRatio"))
				),
				div(style="clear:both;")
			)
		})
		...
		output[[qq("@{experiment_id}_summary_LOH")]] = renderPlot({
			df = df[df$group_name %in% samples, , drop = FALSE]
			par(mar = c(4, 4, 1, 1))
			pie(table(df$LOH))
		})
		...
		output[[qq("@{experiment_id}_summary_TCN")]] = renderPlot({
			df = df[df$group_name %in% samples, , drop = FALSE]
			par(mar = c(4, 4, 1, 1))
			plot(density(log2(df$TCN), na.rm = T), xlab = "log2 TCN", ylab = "Density", main = "")
		})

		output[[qq("@{experiment_id}_summary_CovRatio")]] = renderPlot({
			df = df[df$group_name %in% samples, , drop = FALSE]
			par(mar = c(4, 4, 1, 1))
			plot(density(log2(df$CovRatio), na.rm = T), xlab = "log2 CovRatio", ylab = "Density", main = "")
		})
		...
		output[[qq("@{experiment_id}_heatmap")]] = renderPlot({
			...
			ht = Heatmap(m, name = "log2(CovRatio)", cluster_rows = FALSE, cluster_columns = FALSE, 
	...
	$("#@{experiment_id}_cnv_plot_sample").typeahead({
		...
		observeEvent(input[[qq("@{experiment_id}_cnv_plot_sample_submit")]], {
			...
			gr = gr[width(gr) > 500000]

			seqlevelsStyle(gr) = "UCSC"

			output[[qq("@{experiment_id}_cnv_plot")]] = renderPlot({
				gr$log2CovRatio = log2(gr$CovRatio)
				gr$log2CovRatio[is.na(gr$log2CovRatio)] = 0

				max = max(abs(gr$log2CovRatio))*1.1

				gtrellis_layout(n_track = 1, nrow = 4, compact = TRUE, species = GENOME, 
				    track_ylim = c(-max, max),
				    track_ylab = "log2CovRatio", title = sample,
				    add_name_track = TRUE, add_ideogram_track = TRUE)

				add_segments_track(gr, gr$log2CovRatio, gp = gpar(col = ifelse(gr$log2CovRatio > 0, 2, 3)))
			})
		})
